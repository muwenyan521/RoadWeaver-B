...
public class RoadPathCalculator {
    ...
    public static List<Records.RoadSegmentPlacement> calculateAStarRoadPath(
        ...
        while (!openSet.isEmpty() && maxSteps-- > 0) {
            ...
            if (current.pos.offset(0, -current.pos.getY(), 0).distManhattan(endGround.offset(0, -endGround.getY(), 0)) < neighborDistance * 2) {
            ...
            for (int[] offset : neighborOffsets) {
                ...
                if (elevation > ModConfig.getInstance().maxHeightDifference()) {
                    continue;
                }
                ...
                if (terrainStabilityCost > ModConfig.getInstance().maxTerrainStability()) {
                    continue;
                }
    ...
    private static int heightSampler(int x, int z, ServerLevel serverWorld) {
        long key = hashXZ(x, z);
        return heightCache.computeIfAbsent(key, k -> serverWorld.getChunkSource()
                .getGenerator()
